...
export interface IRepository<T> {
  ...
  /**
   * Unset the database field of the repository.
   *
   * @returns MongoDB database instance
   */
  unsetUpdate(_filter: FilterQuery<T>, data: any): Promise<void>;
}
...
export default class Repository<T> implements IRepository<T> {
  ...
  public async update(_filter: FilterQuery<T>, _data: Partial<T>, _multi?: boolean): Promise<any> {
    return await this.collection.updateOne(_filter, { $set: _data });
  }

  public async unsetUpdate(_filter: FilterQuery<T>, data: any): Promise<any> {
    await this.collection.updateOne(_filter, data);
  }
